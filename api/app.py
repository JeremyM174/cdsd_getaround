...
description = """
Welcome to our car price API! This app is made for the sake of the GetAround project.
...
df = pd.DataFrame({"some": ["data"]}) #reading

loaded_model = joblib.load('/home/user/app/modele_GAR.joblib') #keep file in container - mind workdir and path from copy
...
async def root():
    ...
    return {
        "message": "Welcome to our Car Rental Price Prediction API! 🚗",
        "status": "running",
...
@app.post("/predict", tags=["Predict - machine learning"])
async def predict(predictionFeatures: PredictionFeatures):
    ...
    prediction = loaded_model.predict(variables) #load model as a PyFuncModel

    response = {"prediction": prediction.tolist()[0]} #formatting; to be experimented with for multiple requests
    return response
